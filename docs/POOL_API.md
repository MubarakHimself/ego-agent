...
| `lease_hard_ttl_seconds` | 1800 | Hard lease ceiling |
...
→ `503` when pool at hard K (`{"error":"pool_full"}`).

Idempotent for the same `(agent_id, space_id)` while leased.
...
Also accepted: `POST /v1/leases/{lease_id}/release`.

On release: keep process as `FREE_WARM` if warm count &lt; W, else kill process → `FREE_COLD` (profile remains on disk under `spaces_root`).
