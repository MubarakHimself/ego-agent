...
def _load_or_create_master_key(keyring_mod: Any | None) -> str | None:
...
def build_secret_backend(
    *,
    ...
    key_mat = _load_or_create_master_key(kr)
    if key_mat:
        try:
    ...
    raise VaultUnavailableError(
        ...
        "SLIPSTREAM_VAULT_KEY for tests only"
    )
...
class CredVault:
    ...
    def bind(
        ...
        *,
        ...
        space_id = _require_nonempty_str("space_id", space_id, max_len=200)
        ...
        if not isinstance(cid, str) or not cid.strip():
            raise VaultValidationError("cred_id must be a non-empty string")
        cid = cid.strip()
        if len(cid) > 80:
            raise VaultValidationError("cred_id too long")
    ...
    def unbind(self, space_id: str, cred_id: str) -> dict[str, Any]:
        space_id = _require_nonempty_str("space_id", space_id, max_len=200)
        cred_id = _require_nonempty_str("cred_id", cred_id, max_len=80)
        with self._lock:
            meta = self._read_meta()
...
def parse_fill_body(body: dict[str, Any]) -> dict[str, Any]:
    ...
    for bad in ("secret", "password", "token", "cookie", "cookies", "storage_state"):
        if bad in body:
            raise VaultValidationError(
    ...
    return {"cred_id": cred_id.strip(), "fields": cleaned}
